...
public class GenPlugin extends PluginAdapter
{
    ...
    public boolean sqlMapDocumentGenerated(Document document, IntrospectedTable introspectedTable)
    {
        ...
        // 拼装更新字段
        StringBuilder updateSQL = new StringBuilder("update ").append(tableName).append(" set ").append(pkColumn.getActualColumnName()).append(" = #{item.").append(pkColumn.getJavaProperty()).append("}\n");
        ...
        // rootElement.addElement(createSelect("selectById", tableName,
        // pkColumn));
        ...
        // #{item.").append(pkColumn.getJavaProperty()).append("}");
        // rootElement.addElement(createSql("sql_update",
        // updateSQL.toString()));
        //
        // rootElement.addElement(createUpdate("update"));
        // rootElement.addElement(createUpdate("batchUpdate"));

        // rootElement.addElement(createDels(tableName, pkColumn, "delArray",
        // "array"));
        // rootElement.addElement(createDels(tableName, pkColumn, "delList",
        // "list"));
    ...
    private XmlElement createSave(String id, IntrospectedColumn pkColumn)
    {
        ...
        {
            StringBuilder saveStr = new StringBuilder("<foreach collection=\"list\" index=\"index\" item=\"item\" open=\"\" separator=\";\" close=\"\">\n\t  ").append("<include refid=\"sql_save_columns\" /><include refid=\"sql_save_values\" />\n\t</foreach>");
    ...
    private XmlElement createUpdate(String id)
    {
        ...
        {
            update.addElement(new TextElement("<foreach collection=\"list\" index=\"index\" item=\"item\" open=\"\" separator=\";\" close=\"\">\n\t  <include refid=\"sql_update\" />\n\t</foreach>"));
        }
        return update;
    }
    ...
    private XmlElement createDels(String tableName, IntrospectedColumn pkColumn, String method, String type)
    {
        ...
        StringBuilder deleteStr = new StringBuilder("delete from ").append(tableName).append(" where ").append(pkColumn.getActualColumnName()).append(" in\n\t").append("<foreach collection=\"").append(type)
